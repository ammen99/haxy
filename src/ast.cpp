    ...
    AstNode AstGenerator::generate (AstNodeT root) {
        ...
        for(int i = 1; i < root->children_num - 1; i++) {
            if(std::strstr(root->tag, "number")) {
                ...
                auto node = std::shared_ptr<_AstVariableDeclaration>();
                ...
                for(int i = 1; i < root->children_num; i += 2) {
                    ...
                    if(std::strstr(root->children[i]->tag, "assign")) { // initialize variable
                        auto child = std::shared_ptr<_AstVariableAssignment>();
                        ...
                        auto child = std::shared_ptr<_AstVariable>();
                ...
                auto node = std::shared_ptr<_AstVariableAssignment>();
                ...
                auto node = std::shared_ptr<_AstConditional>();
                node->tag = AstTagConditional;

                auto if_node = std::shared_ptr<_AstIf>();
                ...
                auto node = std::shared_ptr<_AstVariable>();
    ...
    void AstWriter::write(AstNode node, int depth) {
        switch(node->tag) {
            ...
            case AstTagWhile: {
                ...
                write("while:\n", depth);
                write("condition:", depth + 4);
                write(wh->condition, depth + 8);
                write("action = ", depth + 4);
                write(wh->action, depth + 8);
                break;
            }
